...
$api_notice = sprintf(
	/* translators: %s for settings page URL */
...
<table :class="['table-utm-event-tracker-report', {loading: loading}]">
	<thead>
		<tr>
			<th :class="['sortable-column', get_sort_column_class('utm_campaign')]" @click="sort_report('utm_campaign')"><?php esc_html_e('Campaign', 'utm-event-tracker'); ?></th>
			<th :class="['sortable-column', get_sort_column_class('utm_medium')]" @click="sort_report('utm_medium')"><?php esc_html_e('Medium', 'utm-event-tracker'); ?></th>
			<th :class="['sortable-column', get_sort_column_class('utm_source')]" @click="sort_report('utm_source')"><?php esc_html_e('Source', 'utm-event-tracker'); ?></th>
			<th :class="['sortable-column', get_sort_column_class('utm_term')]" @click="sort_report('utm_term')"><?php esc_html_e('Term', 'utm-event-tracker'); ?></th>
			<th :class="['sortable-column', get_sort_column_class('utm_content')]" @click="sort_report('utm_content')"><?php esc_html_e('Content', 'utm-event-tracker'); ?></th>
	...
	<tbody>
		...
		<tr v-for="item in get_data">
			<td class="column-keyword">{{ item.utm_campaign }}</td>
			<td class="column-keyword">{{ item.utm_medium }}</td>
			<td class="column-keyword">{{ item.utm_source }}</td>
			<td class="column-keyword">{{ item.utm_term }}</td>
			<td class="column-keyword">{{ item.utm_content }}</td>
